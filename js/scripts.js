    ...
    $.ajax({url: "ajax-event.php?event=" + parameter, success: function(result){
        ...
        for (i = 0; i < data.length; i++) {
            switch(data[i].niveau_alerte){
                case "1" : res = res + "<a href=\"#\" type=\"button\" class=\"list-group-item\" data-toggle=\"modal\" data-target=\"#alerteModal\" data-titrealerte=\"" + data[i].titre_alerte + "\" data-messagealerte='" + data[i].message_alerte + "' data-localisationalerte='" + data[i].localisation_alerte + "' data-datealerte='" + data[i].date_alerte + "' data-niveaualerte='" + data[i].niveau_alerte + "'><h4 class='list-group-item-heading'>"+ data[i].titre_alerte +"</h4><p class='list-group-item-text'>" + data[i].message_alerte + "</p></a>"; break;
