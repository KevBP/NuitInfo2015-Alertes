$('#alerteModal').on('show.bs.modal', function (event) {
    ...
    $.ajax({url: "getsignalements.php?id="+idalerte, success: function(result){
        ...
        for (i = 0; i < data.length; i++) {
            res = res + "<div class=\"col-lg-3\">Nom : " + data[i].nom_prenom_signalement + "</div><div class=\"col-lg-3\">Age : " + data[i].age_signalement + "</div><div class=\"col-lg-3\">Sexe : " + data[i].sexe_signalement + "</div><div class=\"col-lg-3\">Nom : data[i].description_signalement</div>";
        }
        res = res + "</div>";
